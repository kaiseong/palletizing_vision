...
#
# Usage example:
#     python grabbing_box.py --address 192.168.30.1:50051 --model m
#
...
LIFT_REFERENCE_LINK = "link_torso_5"
...
START_POSE = {
    ...
    "right_arm": [-0.288, -0.400,  0.024, -1.723, -1.040,  1.201,  0.170],
    "left_arm":  [-0.288, 0.400,  -0.024, -1.723, 1.040,  1.201,  -0.170],
    "head":      [0.000,  0.870],
}
...
def build_impedance_grab_command(dyn_model, dyn_state, q):
    ...
    Targets are expressed in IMPEDANCE_REFERENCE_LINK (torso tip). Each hand
    ...
    print(
        f"[grab] ref='{IMPEDANCE_REFERENCE_LINK}' right EEF y: "
    ...
    def arm_impedance(link_name, T_target):
        return (
            ...
            .set_reference_link_name(IMPEDANCE_REFERENCE_LINK)
...
def build_impedance_lift_command(dyn_model, dyn_state, q):
    ...
    print(
        f"[lift] ref='{LIFT_REFERENCE_LINK}' right EEF pos: "
        f"{T_ref2right[:3, 3]} -> {T_right_target[:3, 3]} m"
        f"  (local +z {T_ref2right[:3, 2]})"
    )
    print(
        f"[lift] ref='{LIFT_REFERENCE_LINK}' left  EEF pos: "
    ...
    def arm_cartesian_impedance(link_name, T_target):
        return (
            ...
            .add_target(
                LIFT_REFERENCE_LINK, link_name, T_target,
...
def run_grabbing_sequence(robot, *, monitor_ft=True) -> bool:
    ...
    if model_name != "m":
        raise ValueError(
            f"grabbing_box supports RB-Y1 Model M only; connected model is {model_name!r}"
        )
    ...
    grabbing_done = False  # completion flag
    try:
        ...
        grabbing_done = True
        print("=" * 60)
        print(f"[grabbing] grabbing motion COMPLETED. done = {grabbing_done}")
        print("=" * 60)
    finally:
...
def main(address, model="m", power=".*") -> bool:
    ...
    if normalized_model != "m":
        print(f"Unsupported robot model {model!r}; grabbing_box requires Model M ('m').")
        return False
